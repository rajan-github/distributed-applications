...
public class TFIDF {
    ...
    private static double getInverseDocumentFrequency(String term, Map<String, DocumentData> documentResults) {
        ...
        for (String document : documentResults.keySet()) {
            DocumentData docummentData = documentResults.get(document);
            double termFrequency = docummentData.getFrequency(term);
    ...
    private static double calculateDocumentScore(List<String> terms, DocumentData documentData, Map<String, Double> termToInverseDocumentFrequency) {
        double score = 0.0;
        for (String term : terms) {
            score += (documentData.getFrequency(term) * termToInverseDocumentFrequency.get(term));
        }
        return score;
    }
    ...
    public static Map<Double, List<String>> getDocumentsSortedByScore(List<String> terms, Map<String, DocumentData> documentResults) {
        ...
        Map<String, Double> termToInverseDocumentFrequencyMap = getTermToInverseDocumentFrequencyMap(terms, documentResults);
        for (String document : documentResults.keySet()) {
            double score = calculateDocumentScore(terms, documentResults.get(document), termToInverseDocumentFrequencyMap);
            scoreToDocuments.putIfAbsent(score, new ArrayList<>());
            scoreToDocuments.get(score).add(document);
        }
        return scoreToDocuments.descendingMap();
    }

    public static List<String> getWordsFromLine(String line) {
        //TODO: complete the regex for breaking the line.
